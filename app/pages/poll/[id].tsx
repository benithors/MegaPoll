...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const pollData = await supabaseClient
    ...
    .select("*")
    .eq("id", pollInstanceData.data.poll)
    .single();
  ...
  //for a profile option which is connected to the question
    ...
    const allPollOptions = await supabaseClient
      ...
      .in(
        "poll_question",
        allQuestions.data.map((value) => value.id)
      );
    ...
    const pollAnswersOptions = await supabaseClient
      ...
      .in(
        "poll_option",
        allPollOptions.data.map((value) => value.id)
      );

    let pollOptionWrapper: IPollOptionWrapper[] = [];
    allPollOptions.data.forEach((pollOption) => {
      let pollOptionAnswer = pollAnswersOptions.data.find(
        (answer) => answer.poll_option === pollOption.id
      );
    ...
    for (const pollQuestion of allQuestions.data) {
      const pollQuestionTemp: IPollQuestionWrapper = {
        pollOptionsWrapper: pollOptionWrapper.filter(
...
const Poll = (props: IProps) => {
  ...
  const handleNewOptionsUpdate = (payload: {
    ...
    schema?: string;
    table?: string;
    new: definitions["poll_options_answers"];
    old?: any;
    errors?: string[];
  }) => {
    ...
    props.pollQuestionsWrapper.forEach((question) => {
      question.pollOptionsWrapper.forEach((optionWrapper) => {
        let subTemp = supabaseClient
          .from(
            "poll_options_answers:id=eq." + optionWrapper.pollOptionAnswer.id
          )
          .on("UPDATE", (payload) => {
                  ...
                  {pollQ.pollOptionsWrapper.map((value, idx) => (
                    <div key={idx}>
                      ...
                      <div className={"flex flex-row justify-between"}>
                        ...
                        <div>Votes {value.pollOptionAnswer.votes}</div>
                      </div>
                    </div>
                  ))}
                </div>
